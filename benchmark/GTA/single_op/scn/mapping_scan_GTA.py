...
def mapping_tensorcore(
    ...
    A, B, Scan = scan(M, N, K, in_dtype, out_dtype)
    log_dir = "scan-%s-%s-layer-%s" % (in_dtype, out_dtype, layer)
    log_file = "scan-%s-%s-layer-%s-GTA.log" % (in_dtype, out_dtype, layer)
    target_device = "cuda"
    target_dag = at.compute_dag_from_tensors([Scan])
...
example_text = """
 example:
    python mapping_scan_GTA.py --in_dtype float16 --out_dtype float16 --begin 0 --num 1 --trials 20
    python mapping_scan_GTA.py --in_dtype float16 --out_dtype float32 --begin 0 --num 1 --trials 20
    python mapping_scan_GTA.py --in_dtype float32 --out_dtype float32 --begin 0 --num 1 --trials 20
"""
