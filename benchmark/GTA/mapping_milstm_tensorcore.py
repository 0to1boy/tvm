...
def main(batch, dtype, trials):
    ...
    model = tensor_graph.testing.models.MI_LSTM(
        input_size=input_size, dtype=in_dtype, out_dtype=out_dtype
    )
    model.eval()
    ...
    input_tensor = tensor_graph.core.GraphTensor(input_shape, dtype=in_dtype, name="data")
    old_h = tensor_graph.core.GraphTensor(hc_shape, dtype=in_dtype, name="old_h")
    old_c = tensor_graph.core.GraphTensor(hc_shape, dtype=in_dtype, name="old_h")
    ...
    tid = dispatch.add_graph_task(
        "milstm", multi_graph, measure_opt, scheduler_option="amos_ansor_v2", trials=trials
    )
    ########################################
...
example_text = """
 example:
    python mapping_milstm_tensorcore.py --dtype float16 --trials 20
    python mapping_milstm_tensorcore.py --dtype float32 --trials 20
    python mapping_milstm_tensorcore.py --dtype float64 --trials 200
"""
