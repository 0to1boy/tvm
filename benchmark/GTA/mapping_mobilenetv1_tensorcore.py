...
def main(batch, dtype, trials):
    ...
    model = tensor_graph.testing.models.resnet50(
    ...
    tid = dispatch.add_graph_task(
        "mobilenet", multi_graph, measure_opt, scheduler_option="amos_ansor_v2", trials=trials
    )
    ########################################
    ...
    for i in range(rounds):
        ...
        # print(f"sch_tensors : {sch_tensors}")
        print("get_schedule end...")
        if dispatch.ready(tid):
            print("begin to evaluate whole graph cost...")
            # check if the whole net is runnable
            # if so, evaluate the end-to-end performance
            cost = at.evaluate_graph(multi_graph, sch_tensors, target, 0, 100)
...
example_text = """
 example:
    python mapping_resnet18_tensorcore.py --dtype float16 --trials 20
    python mapping_resnet18_tensorcore.py --dtype float32 --trials 20
    python mapping_resnet18_tensorcore.py --dtype float64 --trials 200
"""
