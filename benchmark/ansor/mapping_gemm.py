...
def run(
    ...
    log_dir = "gemm-%s-%s-layer-%s.json" % (in_dtype, out_dtype, layer)

    target = tvm.target.Target("cuda")
    task = tvm.auto_scheduler.SearchTask(func=gemm, args=(M, N, K, in_dtype, out_dtype), target=target)
...
example_text = """
 example:
    python mapping_gemm.py --in_dtype float16 --out_dtype float16 --begin 0 --num 1 --trials 20
    python mapping_gemm.py --in_dtype float16 --out_dtype float32 --begin 0 --num 1 --trials 20
    python mapping_gemm.py --in_dtype float32 --out_dtype float32 --begin 0 --num 1 --trials 20
"""
