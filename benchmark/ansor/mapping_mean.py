...
def run(
    ...
    log_dir = "mean-%s-%s-layer-%s.json" % (in_dtype, out_dtype, layer)

    target = tvm.target.Target("cuda")
    task = tvm.auto_scheduler.SearchTask(
        func=mean, args=(N, C, H, W, in_dtype, out_dtype), target=target
    )
...
example_text = """
 example:
    python mapping_mean.py --in_dtype float16 --out_dtype float16 --begin 0 --num 1 --trials 20
    python mapping_mean.py --in_dtype float16 --out_dtype float32 --begin 0 --num 1 --trials 20
    python mapping_mean.py --in_dtype float32 --out_dtype float32 --begin 0 --num 1 --trials 20
"""
