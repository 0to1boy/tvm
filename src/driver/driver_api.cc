...
runtime::Module TIRToRuntime(const Map<Target, IRModule>& inputs_arg,
  ...
  for (const auto& it : inputs) {
    if (it.second.defined()) {
      ...
      // 将IRModule分为host_module和device_module
  ...
  // 将device_module的内容导入到runtime::Module中中
...
transform::Sequential MixedModulePassManager(IRModule mixed_mod, Target target) {
  ...
  mixed_pass_list.push_back(tir::transform::AnnotateDeviceRegions()); // TAG: 新注册的后端需要在算子的strategty(wrap_topi_schedule)中绑定线程。否则算子会少一个attr属性，导致无法生成device_module
...
transform::Sequential DeviceModulePassManager(IRModule mixed_mod, Target target) {
  ...
  // Filter 函数用于构造一个用于过滤 tir::PrimFunc 的转换(pass)列表
  ...
  device_pass_list.push_back(tir::transform::RewriteMemoryScope());
