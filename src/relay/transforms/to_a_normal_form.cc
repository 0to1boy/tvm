...
std::pair<NodeScopeMap, ExprSet> CalcScope(const DependencyGraph& dg) {
  ...
  ExprSet lifted_exprs; // 用来保存DependencyGraph里面的跳转指令节点，如Ifnode,Functionnode等
...
Expr ToBasicBlockNormalFormAux(const Expr& e) {
  ...
  support::Arena arena;
  DependencyGraph dg = DependencyGraph::Create(&arena, e);  // 表达式相互依赖的图结构
   ...
   */
  std::pair<NodeScopeMap, ExprSet> scopes = CalcScope(dg);  // 计算每个节点的scope，这个scope可以理解为由跳转指令如IfNode,FunctionNode,LetNode等隔开的子图。
