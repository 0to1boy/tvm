...
void PrintStage(std::ostream* os, int stage_id, const State& state, size_t base_indent,
  ...
  // *os << stage->op->name << " = ... \n";
  if (stage->op.as<te::ComputeOpNode>()->body[0].as<te::ReduceNode>()) {
    *os << stage->op->name << " = " <<  stage->op.as<te::ComputeOpNode>()->body[0].as<te::ReduceNode>()->source[0] << "\n";
  } else {
    *os << stage->op->name << " = " <<  stage->op.as<te::ComputeOpNode>()->body[0] << "\n";
  }
  }
}
