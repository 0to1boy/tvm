...
StepNode* Step::CopyOnWrite() {
  ...
  if (!data_.unique()) {
    if (const auto& ps = as<AnnotationStepNode>()) {
      ...
      ObjectPtr<Object>(std::move(n)).swap(data_);
    } else if (const auto& ps = as<FixedSplitStepNode>()) {
      auto n = make_object<FixedSplitStepNode>(*ps);
      ObjectPtr<Object>(std::move(n)).swap(data_);
    } else if (const auto& ps = as<SetScopeStepNode>()) {
      auto n = make_object<SetScopeStepNode>(*ps);
      ObjectPtr<Object>(std::move(n)).swap(data_);
...
void ReorderStepNode::ApplyToState(State* state) const {
  ...
  state->CopyOnWrite()->stages.Set(
      stage_id, Stage(stage->op, stage->op_type, iters,
      stage->compute_at, stage->attrs, stage->belong_hw_abs));
}
