...
#include "../../src/target/source/ptx.h"
...
bool IsSharedMemoryAllocCorrect(const SearchTask& task, const State& state) {
  // Get the input data type.
  auto type = (task)->compute_dag->tensors[0]->dtype;
  size_t bytes = 1;

  // Record allocate to intrinsic's shared memory size
  size_t intrinsic_size = 0;
  size_t intrinsic_temp = 1;

  // GPU shared memory sizes Oxc000 = 49152.
  size_t SM_size = 49152;
  
  // Control the sum of input data that allocate the shared memory size.
  bool is_next = false;

  if (type == DataType::Float(16) || type == DataType::BFloat(16)) {
    bytes = 2;
  } else if (type == DataType::Float(32)) {
    bytes = 4;
  } else if (type == DataType::Float(64)) {
    bytes = 8;
  } else if (type == DataType::Int(8) || type == DataType::Int(4) || 
                                          type == DataType::Int(1)) {
    bytes = 1;
  }

  // Check weather allocate to GPU shared memory sizes more than Oxc000 = 49152.
  for (size_t i = 0; i < state->stages.size(); ++i) {
    ...
    if (stage->compute_at == ComputeAtKind::kIter && StrEndsWith(stage->op->name, ".shared")) {
      ...
      for (size_t j = 0; j < stage->iters.size(); ++j) {
        ...
        if (const auto* imm = iter->range->extent.as<IntImmNode>()) {
          if (!is_next) {
            intrinsic_temp *= imm->value;
          }
        }
      }
      is_next = true;
    ...
    if (is_next) {
      intrinsic_size += intrinsic_temp;
      intrinsic_temp = 1;
      is_next = false;
    }
  }

  size_t res_size = intrinsic_size * bytes;
...
void PruneInvalidState(const SearchTask& task, Array<State>* states) {
  ...
  for (size_t i = 0; i < states->size(); ++i) {
    ...
    if (IsGPUTask(task)) {
      if (!IsSharedMemoryAllocCorrect(task,(*states)[i])) {
        continue;
      }
    }
