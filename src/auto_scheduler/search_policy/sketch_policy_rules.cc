...
PopulationGenerationRule::ResultKind InitVectorization::Apply(SketchPolicyNode* policy,
  ...
  for (size_t stage_id = 0; stage_id < (*state)->stages.size(); ++stage_id) {
    ...
    if (num_fusible == 1) {
      // Do not select the innermost loop with parameter 1 for vectorization
      if (stage->iters.back()->range.as<RangeNode>()->extent.as<IntImmNode>()->value == 1) {
        return ResultKind::kInvalid;
      }
  ...
  for (int stage_id = (*state)->stages.size() - 1; stage_id >= 0; --stage_id) {
    ...
    if (stage->compute_at == ComputeAtKind::kRoot) {
      ...
      if (total_space_extent <= policy->search_task->hardware_params->warp_size * 2) {
        ...
        ICHECK(!is_warp_level_tensorize);
      ...
      if (reserve_warp) {
        ...
        // policy->search_task->hardware_params->warp_size固定内层循环大小=32
