...
class TouchExtractor : public FeatureVisitor {
 public:
  void Analyze(const Stmt& stmt, const Map<te::Tensor, tir::Buffer> &out_binds) {
    for (auto &bind: out_binds) {
      ...
      this->buffer_info_.insert({buf->data, BufferInfo{GetRef<Buffer>(buf).scope(), shape, buf->dtype}});
  ...
  std::unordered_map<const BufferStoreNode*, InnermostStatementFeature> innermost_stmt_map;
  TvmMap<Var, std::unordered_set<const BufferStoreNode*> > buffervar_stmt_map;
 ...
 private:
  ...
  void ExitItervar_();
  void EnterInnermostStmt_(const BufferStoreNode &innermost_stmt);
  ...
  const BufferStoreNode *current_stmt {nullptr};
