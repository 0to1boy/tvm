...
std::vector<double> Measurer::measure(
  Array<te::Schedule> schedules, Array<te::Tensor> tensors, Target target, DLDevice ctx, double gflop) {
  ...
  auto build_func = [&] (te::Schedule sch, const tvm::Array<te::Tensor>& args, const Target& target) {
    ...
    // );
    return tvm::build(
      tvm::LowerSchedule(sch, args, name, std::unordered_map<te::Tensor, tir::Buffer>(), GlobalVarSupply(NameSupply(""))),
      target,
      Target("llvm")
    );
  };
