...
void TouchExtractor::EnterInnermostStmt_(const BufferStoreNode &innermost_stmt) {
...
void TouchExtractor::VisitStmt_(const BufferStoreNode* op) {
...
void GetInnerStatementFeature(
  ...
  std::vector<const BufferStoreNode*> innermost_stmts;
  ...
  std::sort(innermost_stmts.begin(), innermost_stmts.end(),
            [&](const BufferStoreNode *lhs, const BufferStoreNode *rhs) -> bool {
  ...
  for (auto stmt : innermost_stmts) {
    ...
    buffer << stmt->buffer << "[" << stmt->indices << "] = " << stmt->value;
