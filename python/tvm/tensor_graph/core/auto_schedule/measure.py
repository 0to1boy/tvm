...
def evaluate_function(sch, tensors, target):
  arrays = []
  ctx = tvm.device(target.kind, measure_card_id)
  for t in tensors:
...
def measure_function(index, q=None):
  ...
  print("check context outer:", tvm.device(target.kind, 0).exist)
...
def judge_schedule(schedules, tensors, target, gflop, policy):
  print("check context outer outer:", tvm.device(target.kind, 0).exist)
  if policy == "profile":
    global GLOBAL_BUILD_ARGS
