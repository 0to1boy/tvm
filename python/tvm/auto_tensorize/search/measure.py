...
import sys
from tvm.auto_scheduler.utils import NoDaemonPool
...
def evaluate_schedule(sch, args, measure_opt, new_process=False):
    if not new_process:
        ...
        ctx = (remote if use_rpc else tvm).device(target, dev_id)
        arrays = get_tvm_arrays(args, ctx)
        func = tvm.build(
            sch, args, target=target
        )
        if use_rpc:
    ...
    def timed_func():
        ...
        if error_no == 0:
            ...
            if str(target).startswith("tenet"):
                ...
                func = tenet.build(
                    sch, args, sch_app.tenet_ctx, target=target, name=name
                )
                    ...
                    try:
                        ...
                        with transform.PassContext():
                            func = build_module.build(
                                sch, args, target="cuda", name=name
                            )
                ...
                if enable_perf_model:
                    ...
                    try:
                        ...
                        with transform.PassContext():
                            func = build_module.build(
                                sch, args, target=target, name=name
                            )
                    ...
                    # pylint: disable=broad-except
                        ...
                        error_no = auto_scheduler.measure.MeasureErrorNo.COMPILE_HOST
                        error_msg = auto_scheduler.measure.make_error_msg()
                        print(f"compile host error: {error_msg}",flush=True)
        else:
            filename = ""
...
def pebble_local_builder_build(
    ...
    iterator_count = 0  # xx: record all iterator count
        ...
        if str(target).startswith("tenet"):
            ...
            if parts[1] == "cuda":
                ...
                try:
                    ...
                    ctx = ndarray.device("cuda", dev_id)
                    # Limitation:
            ...
            if enable_perf_model:
                ...
                try:
                    ...
                    ctx = ndarray.device(str(target), dev_id)
                    # Limitation:
                    # We can not get PackFunction directly in the remote mode as it is wrapped
                    # under the std::function. We could lift the restriction later once we fold
                    # the PackedFunc as an object. Currently, we pass function name to work
...
def pebble_local_runner_run(
    ...
    measure_results = []
    buidler_error_count = 0  # record runner error count
    with ProcessPool(n_parallel) as pool:
        ...
        while True:
            try:
                result = next(iterator)
                if auto_scheduler.measure.MeasureResult(*result).error_no != 0:
                    buidler_error_count += 1
            except StopIteration:
                break
            except TimeoutError:
                if verbose >= 1:
