...
def empty_cuda_params_v3():
    return CUDAParamsV3(None, None, [], [], [], None, None)
...
class CUDAScheduleGeneratorV3(AcceleratorScheduleGenerator):
    def __init__(
        ...
        super(CUDAScheduleGeneratorV3, self).__init__(
            eps, CUDAParamsV3, steps=steps, log_file=log_file, verbose_init=verbose_init
        )
    ...
    def init_param_generator(self):
        ...
        for iv in self.output_op.axis[:-reserve_axis_count]:
            # print(f"xx: iv: {iv}, self.spatial_tiling_parts : {self.spatial_tiling_parts}")
            # 求出循环变量能够拆分的因子数组，元素个数为self.spatial_tiling_parts
